...
export function getLarkConfig(req) {
  ...
  const redirectUri = process.env.LARK_REDIRECT_URI || `${getBaseUrl(req)}/api/auth/lark/callback`;
...
export function hasRequiredAuthConfig(req) {
  ...
  return Boolean(config.appId && config.appSecret && process.env.SESSION_SECRET);
}
